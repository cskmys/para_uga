...
void mpi_sync_tx(char *buff, int cnt, int tag, int rxerRank){
    MPI_Ssend(buff, cnt, MPI_CHAR, rxerRank, tag, MPI_COMM_WORLD); // could behave as blocking call if message is too big
}
...
void ring_stx(int myRank, int nbProc, char *buff, int cnt, int tag){
    mpi_sync_tx(buff, cnt, tag, (myRank + 1) % nbProc);
}

void ring_rx(int myRank, int nbProc, char *buff, int cnt, int tag){
    mpi_sync_rx(buff, cnt, tag, (myRank - 1) < 0 ? (nbProc - 1) : (myRank - 1));
}

void ring_chk_for_msg(int myRank, int nbProc, int *cnt, int *tag){
    mpi_chk_for_msg(cnt, tag, (myRank - 1) < 0 ? (nbProc - 1) : (myRank - 1));
}
